...
type diskDedupClient struct {
	client.Client
	zoneDisks map[string][]*computev1.Disk
}

func (c *diskDedupClient) GetZones(_ string) ([]*computev1.Zone, error) {
	zones := make([]*computev1.Zone, 0, len(c.zoneDisks))
	for name := range c.zoneDisks {
		zones = append(zones, &computev1.Zone{Name: name})
	}
	return zones, nil
}

func (c *diskDedupClient) ListDisks(_ context.Context, _ string, zone string) ([]*computev1.Disk, error) {
	return c.zoneDisks[zone], nil
}

func TestDiskStore_Populate_Deduplication(t *testing.T) {
	fakeClient := &diskDedupClient{
		zoneDisks: map[string][]*computev1.Disk{
			"us-central1-a": {
				{Name: "shared-name", Zone: "us-central1-a"},
				{Name: "shared-name", Zone: "us-central1-a"}, // within-zone duplicate: should be dropped
			},
			"us-central1-b": {
				{Name: "shared-name", Zone: "us-central1-b"}, // same name, different zone: should be kept
			},
		},
	}
	...
	assert.Len(t, ds.GetDisks("proj1"), 2,
		"within-zone duplicates should be dropped; cross-zone same-named disks should both be kept")
}
